...
struct VideoSnapshot: Sendable {
    ...
    let nudityDetections: [NudityDetection]
}
...
struct VideoSmoother {
    ...
    mutating func consume(_ raw: VideoSnapshot) {
        ...
        if let prior = curr {
            aligned = Self.alignBodies(curr: raw, to: prior)
        } else {
            aligned = raw
        }
        prev = curr
    ...
    private static func alignBodies(
        ...
        guard currCount > 0 else { return raw }
        ...
        for p in 0..<prevCount {
            if let c = currForPrev[p] { order.append(c) }
        }
        for c in 0..<currCount where !matchedCurr[c] { order.append(c) }
